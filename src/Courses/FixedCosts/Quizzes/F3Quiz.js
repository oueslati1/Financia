...
export default function Quiz() {
  ...
  const question = {
    id: 1,
    question: 'Which of the following best describes the concept of stepped fixed costs?',
    options: [
      "Costs that remain constant regardless of production or sales volume",
      "Costs that fluctuate in direct proportion to changes in production or sales",
      "Costs that increase in discrete steps as activity or output level rises",
    ],
    correctAnswer: 2,
  };
  ...
  useEffect(() => {
    const savedScore = localStorage.getItem('f3quizScore');
    const savedSelectedAnswer = localStorage.getItem('f3selectedAnswer');
  ...
  const f3quizPoints = 15;

  // Save user's answer
  const handleOptionClick = (index) => {
    setSelectedAnswer(index);
    localStorage.setItem('f3selectedAnswer', index.toString());
  };
  ...
  const handleQuizClose = () => {
    ...
    // Update points 
    if (score === 1) {
      setTotalPoints((prevPoints) => prevPoints + f3quizPoints);
      localStorage.setItem('totalPoints', (totalPoints + f3quizPoints).toString());
      window.dispatchEvent(new Event('points-updated'));
    }

    // Save in local storage
    localStorage.setItem('f3quizScore', score.toString());
    localStorage.setItem('F3QuizCompleted', 'true');
  };
