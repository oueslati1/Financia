...
export default function Quiz() {
  ...
  const question = {
    id: 1,
    question: 'What are starting costs?',
    options: ['Costs that change over time', 
    'The initial expenses of setting up a business', 
  ...
  useEffect(() => {
    const savedScore = localStorage.getItem('s1quizScore');
    const savedSelectedAnswer = localStorage.getItem('s1selectedAnswer');
  ...
  const s1quizPoints = 5;

  // Save user's answer
  const handleOptionClick = (index) => {
    setSelectedAnswer(index);
    localStorage.setItem('s1selectedAnswer', index.toString());
  };
  ...
  const handleQuizClose = () => {
    ...
    // Update points 
    if (score === 1) {
      setTotalPoints((prevPoints) => prevPoints + s1quizPoints);
      localStorage.setItem('totalPoints', (totalPoints + s1quizPoints).toString());
      window.dispatchEvent(new Event('points-updated'));
    }

    // Save in local storage
    localStorage.setItem('s1quizScore', score.toString());
    localStorage.setItem('S1QuizCompleted', 'true');
  };
  ...
  const handleQuizReset = () => {
    ...
    setSelectedAnswer(-1);
    setTotalPoints(0);
    localStorage.removeItem('s1quizScore');
    localStorage.removeItem('s1selectedAnswer');
    localStorage.removeItem('S1QuizCompleted');
