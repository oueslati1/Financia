...
export default function Quiz() {
  ...
  const question = {
    id: 1,
    question: 'How do you register a limited company in the UK?',
    options: ['You register a company at your local bank', 
    'You register a company by calling an insurance firm',
    'You register a company with Companies House'],
    correctAnswer: 2,
  };

  // Load saved data from localStorage
  useEffect(() => {
    const savedScore = localStorage.getItem('s2quizScore');
    const savedSelectedAnswer = localStorage.getItem('s2selectedAnswer');
  ...
  const s2quizPoints = 10;

  // Save user's answer
  const handleOptionClick = (index) => {
    setSelectedAnswer(index);
    localStorage.setItem('s2selectedAnswer', index.toString());
  };
  ...
  const handleQuizClose = () => {
    ...
    // Update points 
    if (score === 1) {
      setTotalPoints((prevPoints) => prevPoints + s2quizPoints);
      localStorage.setItem('totalPoints', (totalPoints + s2quizPoints).toString());
      window.dispatchEvent(new Event('points-updated'));
    }

    // Save in local storage
    localStorage.setItem('s2quizScore', score.toString());
    localStorage.setItem('S2QuizCompleted', 'true');
  };
  ...
  const handleQuizReset = () => {
    ...
    setSelectedAnswer(-1);
    setTotalPoints(0);
    localStorage.removeItem('s2quizScore');
    localStorage.removeItem('s2selectedAnswer');
    localStorage.removeItem('S2QuizCompleted');
  };
