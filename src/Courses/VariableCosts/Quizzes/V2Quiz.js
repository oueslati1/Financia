...
export default function Quiz() {
  ...
  const question = {
    id: 1,
    question: 'A company charges clients £500 per hour, its variable costs is £75 per hour. What is the variable cost ratio?',
    options: ['0.15', 
    '0.4', 
    '0.25'],
    correctAnswer: 0,
  ...
  useEffect(() => {
    const savedScore = localStorage.getItem('v2quizScore');
    const savedSelectedAnswer = localStorage.getItem('v2selectedAnswer');
  ...
  const v2quizPoints = 10;

  // Save user's answer
  const handleOptionClick = (index) => {
    setSelectedAnswer(index);
    localStorage.setItem('v2selectedAnswer', index.toString());
  };
  ...
  const handleQuizClose = () => {
    ...
    // Update points 
    if (score === 1) {
      setTotalPoints((prevPoints) => prevPoints + v2quizPoints);
      localStorage.setItem('totalPoints', (totalPoints + v2quizPoints).toString());
      window.dispatchEvent(new Event('points-updated'));
    }

    // Save in local storage
    localStorage.setItem('v2quizScore', score.toString());
    localStorage.setItem('V2QuizCompleted', 'true');
  };
  ...
  const handleQuizReset = () => {
    ...
    setSelectedAnswer(-1);
    setTotalPoints(0);
    localStorage.removeItem('v2quizScore');
    localStorage.removeItem('v2selectedAnswer');
    localStorage.removeItem('V2QuizCompleted');
  };
