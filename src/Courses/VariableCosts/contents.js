...
export default function Contents() {
  ...
  useEffect(() => {
    ...
    switch (location.pathname) {
      case '/VariableCosts':
        ...
        break;
      case '/vchapter1':
        setCurrentChapterForToggle('Chapter 1: Introduction to Variable Costs');
        break;
      case '/vchapter2':
        setCurrentChapterForToggle('Chapter 2: Calculating Variable Costs');
        break;
      case '/vchapter3':
        setCurrentChapterForToggle('Chapter 3: Economies of Scale');
        break;
      case '/nextsteps':
  ...
  const calculateProgress = () => {
    const completionStatus = {
      chapter1: localStorage.getItem('V1QuizCompleted') === 'true',
      chapter2: localStorage.getItem('V2QuizCompleted') === 'true',
      chapter3: localStorage.getItem('V3QuizCompleted') === 'true',
    };

    const chapterTitles = {
      introduction: 'Introduction',
      chapter1: 'Chapter 1: Introduction to Variable Costs',
      chapter2: 'Chapter 2: Calculating Variable Costs',
      chapter3: 'Chapter 3: Economies of Scale',
      ...
      {showDropdown && (
        <div className={styles['dropdown-menu']}>
          <button
            ...
            onClick={() => handleDropdownItemClick('/variablecosts', 'Introduction')}
          >
            Introduction
          </button>
          ...
          <button
            ...
            onClick={() => handleDropdownItemClick('/vchapter1', 'Chapter 1: Introduction to Variable Costs')}
          >
            {getChapterIcon('chapter1')}
            {formatChapter('Chapter 1: Introduction to Variable Costs')}
          </button>

          <button
            className={styles['dropdown-item']}
            style={{ borderRadius: '0' }}
            onClick={() => handleDropdownItemClick('/vchapter2', 'Chapter 2: Calculating Variable Costs')}
          >
            {getChapterIcon('chapter2')}
            {formatChapter('Chapter 2: Calculating Variable Costs')}
          </button>
          <button
            className={styles['dropdown-item']}
            style={{ borderBottom: '1px solid #ddd', borderRadius: '0' }}
            onClick={() => handleDropdownItemClick('/vchapter3', 'Chapter 3: Economies of Scale')}
          >
            {getChapterIcon('chapter3')}
            {formatChapter('Chapter 3: Economies of Scale')}
          </button>
          <button
            className={styles['dropdown-item']}
            style={{ fontWeight: 'bold', height: '60px', borderBottomRadius: '10px' }}
            onClick={() => handleDropdownItemClick('/VNextsteps', 'Next Steps')}
          >
            Next Steps
          </button>
        </div>
